        ...
        if _twitch_match:
            ...
            async with session.get(
                    'https://www.googleapis.com/youtube/v3/videos?part=contentDetails%2C+snippet%2C+statistics&id={}&key={}'.format(
                        _id, 'AIzaSyBeN3PHoLnDg0VrMej568SNr--vv8CVTjA')) as resp:
                assert resp.status == 200
                resp_parsed = await resp.json()
                if resp_parsed.get('error'):
                    if resp_parsed['error']['code'] == 403:
                        await self.bot.send_message(message.channel, 'Failed to fetch content from youtube :(')
                    else:
                        pass
                ...
                channel = discord.utils.get(self.bot.get_all_channels(), name='youtubestuff')
                if message.channel.id != channel.id:
                    await self.bot.send_message(channel, 'https://{}'.format(_youtube_match.group(0)))


def setup(bot):
    n = Web_info(bot)
    bot.add_listener(n.process_always, "on_message")
    bot.add_cog(n)